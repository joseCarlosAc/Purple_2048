...
export{makeRequest,editUser,initData,login,bestScores,loadGames,leaderBoard,createUser,logout};
...
async function createUser(){
    try{
        ...
        let newUser= await makeRequest("POST","/api/users",[
...
async function putLogin(data){
    try{
        let token= await makeRequest("PUT","/api/login",[
            {"name":"Content-Type","value":"application/json"}
        ],data);
